...
public class Ball extends Actor
{
    ...
    public void act() 
    {
        ...
        else
        {
            ...
            checkRestart();
            isTouchingPaddles();
            if(this.hitPaddle == 10)
            {
                speed += 1;
                hitPaddle = 0;
            }
            if(getY() < getWorld().getHeight()/9*7)
                reverted = false;
        }
    }    
    ...
    private void isTouchingPaddles()
    { 
        PlayerPaddle getInfo = (PlayerPaddle)this.getOneIntersectingObject(PlayerPaddle.class);
        if(getInfo!=null)
        {
            if(!reverted)
            {
                Greenfoot.playSound("caughtball.wav");
                revertVertically();
                reverted = true;
                this.hitPaddle += 1;
            }
        }
        else if(this.getOneIntersectingObject(BotPaddle.class)!=null && getRotation() > 180 && getRotation() < 359)
        {
            Greenfoot.playSound("zap.wav");
            getWorld().removeObject(this.getOneIntersectingObject(BotPaddle.class));
            getWorld().showText("Score: "+(this.score += 10), 700, 50);
            revertVertically();
        }
    }
    ...
    private void checkBounceOffCeiling()
    {
        if (isTouchingCeiling())
        {
            if (! hasBouncedVertically)
            {
                ...
                this.hit += 1;
                if(hit%10 == 0){
                    ((PingWorld)this.getWorld()).lvlUp();
                    getWorld().showText("Score: "+(this.score += 100), 700, 50);
                }
                revertVertically();
            }
        }
